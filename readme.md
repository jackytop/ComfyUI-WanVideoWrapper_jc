...
- Multiple reference images for WanAnimate and Wan-Animate-2 (experimental, the models aren't trained for it): `prefix_frames` takes up to 5 extra images of the character. `WanVideo Animate Embeds` places them as extra reference latents in front of the main one (`prefix_mode` single_frame) or as known frames on an expanded canvas in front of the video that is trimmed afterwards (canvas); `WanVideo Animate2 Embeds` puts them as known frames right after the reference slot. Works with context windows and the windowed/looped generation.
- `sageattn_mem_eff` attention mode (after KJNodes' WanVideo Mem Eff Sage Attention Patch): the same SageAttention kernels, but the bf16 q/k/v are freed as soon as they're quantized, about 0.9 GB less peak per attention at ~480x864x81 (sm80/86/89/120, other GPUs fall back to `sageattn`). Also covers Wan-Animate-2's generation attention (pose frames and `log_scale` bias), where it saves about 0.85 GB more. Combine with the sampler's `rope_function` `comfy_chunked`, which chunks the feed forward (the KJNodes Wan Chunk FeedForward equivalent).
